...
void system_destroy(int signo)
{
	close(camera_fd);
	int n;
	for(n=0;n<COUNT_NUM;n++)
	{
...
void sig_handler(int signo)
{
	if(signo==SIGPIPE)
	{
		...
		printf("somebody close the explore\n");
	}
}
void *g_th(void *arg)
{
...
{	struct v4l2_capability cap;
	...
	pix_format = fmat.fmt.pix.pixelformat;
}
...
void dev_on()
{
	...
	if(pthread_create(&th,&attr,g_th,NULL)!=0)
	{
	...
	if(signal(SIGINT,system_destroy)==SIG_ERR)
	{
		fprintf(stderr,"signal: %s\n",strerror(errno));
		exit(1);
	}
...
//二级缓存动态分配内存，并返回空间大小
static int set_temp_size()
{
	int size = WIDTH*HEIGHT*2;
	switch(pix_format){
		case MJPEG:{
			temp_buf = (unsigned char *)calloc(1,(size_t)size);
			if(!temp_buf)
				return -1;
			break;
		}
		...
		default:{
			fprintf(stderr,"Camera format unsuported!\n");
			return -1;
			break;
		}
	}
...
void get_picture_th()
{
	...
	while(1)
	{
		...
		{
			case MJPEG:{
						   memcpy(temp_buf,buffers[i].start,temp_len);
						   //printf("dqbuf success,index: %d\n",i);
						   break;
					   }
			case YUYV:{
						  yuv_to_jpeg(buffers[i].start,temp_buf,temp_len,62);
						  break;
					  }
			default:{
						printf("unknown pix format\n");
						break;
					}
		}
        usleep(100000);
...
void send_picture_th(int clientfd)
{
...
void send_picture(int clientfd)
{
	char *http = "HTTP/1.0 200 OK\r\nConnection: close\r\nServer: Net-camera-1-0\r\nCache-Control: no-store, no-cache, must-revalidate, pre-check=0, post-check=0, max-age=0\r\nPragma: no-cache\r\nContent-type: multipart/x-mixed-replace;boundary=www.briup.com\r\n\r\n";
	int len=strlen(http);	
	if(write(clientfd,http,len)!=len)
