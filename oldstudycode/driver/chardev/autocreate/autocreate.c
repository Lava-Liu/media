...
static int __init char_init(void)
{
	...
	//自动创建设备类别属性
	global_devp->gd_cls = class_create(THIS_MODULE, DEV_NAME);
	if(global_devp->gd_cls == NULL)
	{
		printk(KERN_ERR"class_create error!\n");
		goto err1;
	}
	//封装struct class 类中的设备属性
	global_devp->gd_cls->dev_attrs = dev_attr;
	//在指定的类中创建设备子文件管理系统
	global_devp->gd_dev = device_create(global_devp->gd_cls, NULL, devno, NULL, DEV_NAME"%d", MINOR(devno));
	if(global_devp->gd_dev == NULL)
	{
	...
	return 0;
err0:
	class_destroy(global_devp->gd_cls);
err1:
...
{
	device_destroy(global_devp->gd_cls, MKDEV(major_num, 0));
	class_destroy(global_devp->gd_cls);
	cdev_del(&global_devp->cdev);
	kfree(global_devp);
	global_devp=NULL;
