...
{
	ssize_t ret=0;
	printk(KERN_INFO"myshow() invoked!\n");
	printk("myshow_buf: %s\n", buf);
	printk("myshow_buf: %p\n", buf);
	return ret;
}
...
{
	ssize_t ret =0;
	printk(KERN_INFO"mystore() invoked!\n");
	printk("mystore_buf: %s\n", buf);
	printk("mystore_buf: %p\n", buf);
	memcpy(global_devp->myattr, buf, strlen(buf));
	return ret;
}


//DEVICE_ATTR(myattr, S_IRWXUGO , NULL, NULL);
struct device_attribute dev_attr[]=
{
	__ATTR(myattr, S_IRWXUGO, myshow, mystore), 
	__ATTR_NULL, 
...
static int __init char_init(void)
{
	...
	//2/给自定义设备的描述分配内存
	global_devp = (global_dev*)kmalloc(sizeof(global_dev)*counter, GFP_KERNEL);
	if(global_devp == NULL)
	{
	...
	memset(global_devp, 0, sizeof(global_dev)*counter);
	...
	//封装struct class 类中的设备属性
	global_cls->dev_attrs = dev_attr;
	//初始化设备描述结构体
	for(i=0;i<counter;i++)
	{
		char_setup(&global_devp[i], MKDEV(major_num, i));
		//在指定的类中创建设备子文件管理系统
		global_devp[i].gd_dev = device_create(global_cls, NULL, MKDEV(major_num, i), NULL, DEV_NAME"%d", i);
		if(global_devp[i].gd_dev == NULL)
		{
...
static void __exit char_exit(void)
{
	...
	for(i =0;i<counter;i++)
	{
	device_destroy(global_cls, MKDEV(major_num, i));
	...
	for(i = 0;i<counter;i++)
	{
	unregister_chrdev_region(MKDEV(major_num, i), counter);
	}
