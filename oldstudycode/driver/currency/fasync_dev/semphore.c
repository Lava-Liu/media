...
static ssize_t char_write(struct file* file ,const char __user *buffer, size_t size, loff_t *offset)
{
	...
	*offset += ret;
	...
	if(devp->cur_len>0)
	{
		kill_fasync(&devp->fa_table, SIGIO, POLL_IN);
	}
	return ret;
}
...
int char_fasync(int fd, struct file* file , int mode)
{
	//注册设备的异步io支持
	printk(KERN_INFO"char_fasync() invoked!\n");
	return fasync_helper(fd, file, mode, &devp->fa_table);
}

static int char_close(struct inode* inode, struct file* file)
{
	//注销设备的异步支持
	char_fasync(-1, file, 0);
	printk(KERN_INFO"char_close() invoked\n");
	return 0;
}

static unsigned int char_select(struct file* file, struct poll_table_struct *table)
{
	unsigned int mask = 0;
	//将引起文件描述状态变化的等待队列添加到轮循表
	poll_wait(file, &devp->r_wait, table);
	poll_wait(file, &devp->r_wait, table);
	//函数返回读写状态的掩码
	if(devp->cur_len > 0)
	{
