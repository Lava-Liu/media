...
void init()
{
	...
	while(1)
	{
		...
		if((clientfd[clientid] = accept(sockfd,(struct sockaddr *)&addr,&clientlen))<0)
		{
			fprintf(stderr,"sockfd: %s\n",strerror(errno));
			exit(1);
		}
		...
		if(i==0)
		{
			...
			ssize_t n_read;
			char buffer[4096]={0};
			if((n_read=read(clientfd[clientid],buffer,4096))<0)
			{
		...
		pthread_create(&th[i],&attr,tcp_server,(void *)clientfd[clientid]);
